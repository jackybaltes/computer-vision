...
Our goal is to develop an efficient and simple algorithm for 
estimating the angular and linear velocities of a moving robot.

The idea is that instead of assuming a 3D world scene, we assume that the
...
In effect, we assume that our camera is looking at a large image of
